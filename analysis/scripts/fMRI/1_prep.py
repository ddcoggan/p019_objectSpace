...
for subject in experiment['scanInfo'].keys():
    for s, session in enumerate(experiment['scanInfo'][subject].keys()):
        ...
        freesurferSubjectDir = f'/mnt/HDD12TB/freesurfer/subjects/{subject}'
        if not os.path.isdir(os.path.join(freesurferSubjectDir)):
            ...
            freesurferCommand = f'recon-all -subject {subject} -all -hires -i {inFile} -expert expert.opts'
        ...
        for scan in experiment['scanInfo'][subject][session]['funcScans'].keys():
            ...
            for r, run in enumerate(experiment['scanInfo'][subject][session]['funcScans'][scan]):
                ...
                funcDir = os.path.join('data/fMRI/individual', subject, session, 'functional', scan, f'run{r+1:02}')
                ...
                if not os.path.isfile(outFile):
                    ...
                    os.system(f'python2 /mnt/HDD12TB/masterScripts/fMRI/fsl_TOPUP_call.py {inFile} {topupFile} 90 270')
                ...
                if scan != 'restingState':
                    eventDir = os.path.join('data/fMRI/events/3column', subject, session, scan, f'run{r+1:02}')
                    ...
                    eventData = logData['experiment']['conditions']
                    blockOrder = []
                    for b in range(nBlocks):
                        thisCond = []
                        item = eventData[0, 0][0][b][0][0]
                    ...
                    for c, cond in enumerate(condNames):
                        ...
                        thesePositions = [np.where(np.array(blockOrder) == c)][0][0]
                        with open(eventFile, 'w+') as file:
                            for p in thesePositions:
                                start = int(params.initialFixation + p * (params.blockDuration + params.IBI))
                ...
                for HRFmodel in ['singleGamma', 'doubleGamma']:

                    if scan != 'restingState':
                        designDir = os.path.join('data/fMRI/designs/firstLevel', scan, HRFmodel)
                        os.makedirs(designDir, exist_ok=True)

                        for topup in ['noTopUp', 'topUp']:
                            for b0 in ['noB0', 'b0']:

                                outDir = os.path.join('data/fMRI/individual', subject, session, 'functional', scan, f'run{r+1:02}/outputs', topup, b0, HRFmodel)
